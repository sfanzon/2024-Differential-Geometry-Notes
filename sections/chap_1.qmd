...
::: Important
Reparametrizations are important for the following reason: For a curve $\g$ we wish to find a reparametrization 
$\tg$ which is unit speed:
$$
...
$$
This is parametrized by the curves $\g, \eeta \ \colon (-1,1) \to \R^2$ defined by
$$
\g (t) = (t,t^2) \,, \quad 
\eeta(t) = (t^3, t^6) \,.
$$
We claim that $\eeta$ cannot be a reparametrization of $\g$.
...
- $\g$ is a regular parametrization, as the first component of $\dg$ is non-zero.
...
$$
Moreover $\f$ is a diffeomorphism, so that 
$$
\dot \f (t) \neq 0 \,, \quad \forall\, t \in (\tilde{a},\tilde{b})  \,,
$$
by Lemma \ref{lemma-diffeomorphism}. From (@eq-regularity-invariant-1) we conclude that 
$$
\dtg(t) \neq  0 \, , \quad \forall \, t \in (\tilde{a},\tilde{b}) \,, 
$$
...
\dot{s}(t) = \norm{ \dg (t) } \,.
$$
Therefore if $\g$ is regular we have
$$
\norm{ \dg (t) } \neq 0  \quad \implies \quad
\dot{s}(t) \neq 0 \,.
$$
It seems like $s$ could be a diffeomorphism! Indeed, we will prove that if $\g$ is regular then $s$ is a diffeomorphism, and all the unit speed reparametrizations of $\g$ are of the form
$$
\f = \pm s^{-1}  + c 
$$ 
with $s^{-1}$ the inverse of $s$ and $c \in \R$ constant. This essentially means that $s^{-1}$ is the *only* unit speed reparametrization, given that all the others are obtained either 

- by constant time-shift $c$,
- or by time reversal, taking $\phi = - s^{-1}$.

These statements will be proved in Theorem \ref{theorem-regular-unit-speed} and Proposition \ref{proposition-arc-length-unit} below.
...
$$
Since $s$ is a scalar function, the above condition and the Inverse Function Theorem guarantee the existence of a smooth inverse
$$
...
::: Proof
*Step 1. First Point.* 
Assume $\tg$ is a unit speed reparametrization of $\g$ (note that $\tg$ exists by Theorem \ref{theorem-regular-unit-speed}, since $\g$ is assumed to be regular). Then
there exists a reparametrization map $\f$ such that
$$
\g(t) = \tg ( \f(t) )  , \quad \forall \, t \in (a,b)\,.
$$
...
\dot s(t) = - \dot \f(t)   \quad  \forall \, t \in (a,b)
$$ 
Integrating the last two equations we obtain the thesis
$$
\f = \pm s + c
$$
...
*Step 2. Second Point.*  Suppose that $\tg$ is a reparametrization of $\g$, that is,
$$
\g(t) = \tg (\f(t)) \,, \quad \forall \, t \in (a,b) 
$${#eq-proposition-unit-speed-2tris}
for some diffeomorphism $\f \colon (a,b) \to (\tilde{a}, \tilde{b})$.
...
definition of $s$. Taking the absolute values yileds
$$
\norm{\dg(t)} = \norm{ \dg(\f(t)) } \norm{ \dg(t)} \,.
$$
...
::: Remark
Let $\g$ be regular. Proposition \ref{proposition-arc-length-unit} tells us that the following are equivalent:

1. Computing a unit speed reparametrization $\tg$ of $\g$,

2. Computing $s^{-1}$ where $s$ is the arc-length of $\g$.

In some cases however, unit speed reparametrization and arc-length are impossible to characterize in terms of elementary functions. This can happen even for very simple curves.
:::
...
$$
Since $\g$ is regular, by Theorem \ref{theorem-regular-unit-speed} and Proposition \ref{proposition-arc-length-unit} we know that $\g$ admits a unit speed reparametrization $\tg$ such that
$$
\g =  \tg \circ \f
$$
...
$$
for some $c \in \R$. It can be shown that the above integral cannot be written in terms of elementary functions. Therefore the unit speed parametrization $\tg$ cannot be computed explicitly by inverting $s$.
:::
