...
![Interpretation of $\norm{\uu-\vv}$ in $\R^2$](/images/length_difference.png){width=70%}
...
**Question.** The circle of radius $R$ is parametrized by $\g \ \colon [0,2\pi] \to \R^2$ defined by 
$$
\g(t) = (R\cos(t), R\sin(t)) \,.
$$
Prove that $L(\g) = 2\pi R$.
...
**Question.** Consider one full turn of the Helix of radius $R >0$ and rise $H$. This is parametrized by
$$
\g(t) = (R\cos(t), R\sin(t) ,Ht)
$$
for $t \in (0,2\pi)$. Compute $L(\g)$.
...
$$
s(t)=\int_{t_{0}}^{t}\|\dg(u)\| d u \,.
$$
The function $s(t)$ is known as the arc-length function.
...
$$
\g(t) = (R\cos(t), R\sin(t),0) \,, \quad  t \in [0,2\pi] \,.
$$
...
**Question.** The Logarithmic spiral is defined by $\g \ \colon [0,2\pi] \to \R^2$ with
$$
\g(t) = (e^{kt} \cos(t), e^{kt} \sin(t)) \,,
$$
where $k \in \R$, $k \neq 0$, is called the **growth factor**. Compute the arc-length function of $\g$. 

**Solution.** We have
\begin{align*}
\dg(t) & = e^{kt} ( k \cos(t) - \sin(t), k \sin(t) + \cos(t) ) \\
...
Let $\g \ \colon (a,b) \to \R^n$ be a curve. A **reparametrization** of $\g$ is a curve $\tilde{\g} \ \colon (\tilde{a},\tilde{b}) \to \R^n$ such that
$$
...
![Sketch of 1D shape $\Gamma$ parametrized by $\g$ and $\tg$. We can switch parametrization by means of the diffeomorphism $\f$.](/images/reparametrization.png){#fig-reparametrization width=70%}
...
::: Definition
### Regular points  {#definition-regular}
...
3. The curve $\g$ is regular if every point of $\g$ is regular, that is,
$$
...
![Left: Animation of $\g(t) = (t,t^2)$. Right: Animation of $\eeta(t) = (t^3,t^6)$.](/images/singular_curve_animated.gif){width=90%}
...
Let $\g \colon (a,b) \to \R^n$ be a curve. A **unit-speed reparametrization** of $\g$ is a reparametrization $\tg \colon (\tilde{a},\tilde{b}) \to \R^n$ which is unit-speed, that is,
$$
...
$$
The above statements will be proved in Theorem \ref{theorem-regular-unit-speed} and Proposition \ref{proposition-arc-length-unit} below.
...
::: Example 
### Arc-length reparametrization of the Circle {#example-circle-arc}

**Question.** Let $\g \colon (0,2\pi) \to \R^2$ be the circle
with center $(x_0,y_0)$ and radius $R>0$
$$
...
$${#eq-twisted-arc}
Since $\g$ is regular, by Proposition \ref{proposition-arc-length-unit} 
