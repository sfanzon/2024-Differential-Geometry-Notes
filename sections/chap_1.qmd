...
Let $\g \ \colon (a,b) \to \R^n$ be a curve. We say that $\g$ is a **unit-speed** curve if
$$
...
for a suitable diffeomorphism
$$
\f \colon (\tilde{a},\tilde{b}) \to (a,b) \,.
$$
We call both $\f$ and $\f^{-1}$  **reparametrization maps**.
:::
...
Let $\g \colon (a,b) \to \R^n$ be a parametrized curve. A **unit-speed reparametrization** of $\g$ is a reparametrization $\tg \colon (\tilde{a},\tilde{b}) \to \R^n$ which is unit-speed, i.e.,
$$
...
1. $\g$ is regular,
2. $\g$ has a unit-speed reparametrization.
:::
...
Let $\g$ be regular. Proposition \ref{proposition-arc-length-unit} tells us that a unit 
speed reparametrization of $\g$ is given by
$$
\tg := \g  \circ s^{-1}
$$
with $s^{-1}$ inverse of the arc-length of $\g$. We call $\tg$ the **arc-length reparametrization** of $\g$.
...
Let $\g \colon [0,2\pi] \to \R^2$ be the parametrization of the circle
with center $(x_0,y_0)$ and radius $R>0$
$$
\g(t) = ( x_0 + R \cos (t), t_0 + \sin (t))
$$
The arc-length of $\g$ starting from $t_0 = 0$ is
$$
s(t) = \int_0^t \norm{ \dg(\tau) } \, d\tau = Rt
$$
...
t(s) = \frac{s}{R} 
$$
By Proposition \ref{proposition-arc-length-unit} 
$$
\tg := \g \circ s^{-1} 
$$
is a unit-speed reparametrization of $\g$.
Plugging the formula for $s^{-1}$ into the expression for $\g$ 
yields the explicit formula for $\tg$
$$
\tg(s) = \g(t(s)) = \left( x_0 + R \cos \left( \frac{s}{R} \right), y_0 + \sin \left( \frac{s}{R} \right) \right) 
$$
