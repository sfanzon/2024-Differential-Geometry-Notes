...
Curves are 1D objects in the 2D or 3D space. For example in two dimensions one could think of a straight line, a hyperbole or a circle. These can be all described by an equation in the $x$ and $y$ coordinates: respectively
$$
...
We will denote the first, second and third derivatives of $f$ as follows:
$$
\dot f :=  \frac{d f}{dt} \,, \quad \ddot f := \frac{d^2 f}{dt^2} \,, 
\quad \dddot f := \frac{d^3 f}{dt^3} \,.
$$
...
![Example of smooth and non-smooth curves](/images/smooth_curve.png){#fig-smooth-curve width=40%}
...
![Approximating the tangent vector](/images/tangent.png){#fig-tangent width=40%}
...
For a vector $\vv \in \R^n$ with components 
$$
\vv=(v_1,\ldots,v_n),
$$
its **length** is defined by
$$
\norm{\vv}:= \sqrt{\sum_{i=1}^n v_i^2 } \,.
$$
The above is just an extension of the Pythagoras theorem to $\R^n$, and the length of $v$ is computed from the origin.


![Interpretation of $\norm{v}$ in $\R^2$](/images/pythagoras.png){width=40%}


If we have a second vector $\uu \in \R^n$, then the quantity 
$$
\norm{\uu-\vv}:= \sqrt{\sum_{i=1}^n (u_i-v_i)^2 } 
$$
measures the length of the difference between $u$ and $v$.


![Interpretation of $\norm{\uu-\vv}$ in $\R^2$](/images/length_difference.png){width=40%}
...
![Approximating the length of $\g$](/images/length_approx.png){width=40%}
...
![Approximating $L(\g)$ via $\dg$](/images/length_derivative.png){width=40%}
...
Let $\mathcal{P} = (t_0, \ldots, t_m)$ be a partition of $[a,b]$ with $\norm{\mathcal{P}} < \delta$. Recall that 
$$
...
Note that if $H>0$ then
$$
2\pi \sqrt{R^2 + H^2} > 2\pi R \,,
$$
showing that the length of one full turn of the Helix
is larger than the length of a disk. This might seem
counterintuitive as it looks like one turn of the Helix
can be superimposed to the circle by *squashing* the Helix
on the plane. However this *squashing* action clearly 
causes a bit of shrinkage, as shown in the above estimate.
...
We have just shown in Theorem \ref{theorem-length} that the length of a smooth curve $\g \ \colon [a,b] \to \R^n$ with $[a,b]$ bounded is given by
$$
L(\g) = \int_a^b \norm{ \dg (t)} \,dt \,.
$$
...
![Arc-length of $\g$ starting at $\g(t_0)$](/images/arc_length.png){width=40%}
...
**Question.** Consider the circle of radius $R$, parametrized by
$$
\g(t) = (R\cos(t), R\sin(t),0)  \,.
$$
Compute the arc-length function of $\g$.

**Solution.** We have
$$
\dg(t) = (-R\sin(t), R\cos(t),0) \,, \quad \norm{\dg(t)} = R \,.
$$
Therefore, for any fixed $t_0$, we have
$$
...
::: Definition
### Scalar product in $\R^2$  {#definition-scalar}

Let $\uu, \vv \in \R^2$ and denote by $\theta \in [0,\pi]$ the angle formed by $u$ and $v$. The *scalar product* between $\uu$ and $\vv$ is defined by 
$$
...
![Vectors $u$ and $v$ in $\R^2$ forming angle $\theta$](/images/scalar_product.png){width=40%}


::: Remark 

1. Two vectors in the plane form two complementary angles. To avoid ambiguity, we choose the smallest of the two angles. This is enforced in Definition \ref{definition-scalar} by requiring that $\theta \in [0,\pi]$.

2. The scalar product is maximized for $\theta = 0$, for which we have
$$
\uu \cdot \vv = \|\uu\| \|\vv\| \cos(\theta) =  \|\uu\| \|\vv\| \,.
$$

3. It is instead minimized for $\theta = \pi$, for which 
$$
\uu \cdot \vv = \|\uu\| \|\vv\| \cos(\theta) =  - \|\uu\| \|\vv\| \,.
$$

4. For each $\uu,\vv \in \R^2$ the Cauchy-Schwarz inequality holds: 
$$
-\|\uu\| \|\vv\| \leq  \uu \cdot \vv  \leq \|\uu\| \|\vv\|
$$
The above is immediate from the observation that $|\cos(\theta)| \leq 1$.


5. The observations in points 2-3 imply that the Cauchy-Schwarz inequality is sharp, in the sense that both inequalities are attained.

6. Usually the Cauchy-Schwarz inequality is written in the equivalent form
$$
| \uu \cdot \vv | \leq \| \uu \| \| \vv \|
$$

7. By the above observations it follows that equality holds if
and only if $\uu$ and $\vv$ are parallel.
...
Let $\uu, \vv \in \R^2$. We say that $\uu$ and $\vv$ are **orthogonal** if
$$
\uu \cdot \vv = 0
$$
...
Let $\uu, \vv, \ww \in \R^2$ and $\lambda \in \R$. Then
...
- **Bilinearity**: They hold
$$
\lambda (\uu \cdot \vv) = (\lambda \uu) \cdot \vv = \uu \cdot (\lambda \vv) \,,
$$
$$
...
The above proposition is saying that the scalar product is **bilinear** and **symmetric**. We leave the proof to the reader: only the condition
$$
\uu \cdot (\vv + \ww) = \uu \cdot \vv  + \uu \cdot \ww \,.
$$
is non-trivial, due to the presence of 3 vectors.
...
Denote by 
$$
\ee_1 = (1,0) \,, \quad \ee_2 = (0,1)
$$
the euclidean basis of $\R^2$. Let $\uu, \vv \in \R^2$ and denote by 
$$
\uu = (u_1,u_2) = u_1 \ee_1 + u_2 \ee_2 
$$
$$
\vv = (v_1,v_2) = v_1 \ee_1 + v_2 \ee_2 
$$
their coordinates with respect to $\ee_1, \ee_2$. Then
$$
\uu \cdot \vv = u_1v_2 + u_2 v_2 \,.
$$
:::
...
Note that 
$$
\ee_1 \cdot \ee_1 = 1 \,, \quad \ee_2 \cdot \ee_2 = 1 \,, \quad 
\ee_1 \cdot \ee_2 = \ee_2 \cdot \ee_1 = 0 \,.
$$
Using the bilinearity of scalar product we have
\begin{align*}
\uu \cdot \vv & = (u_1 \ee_1 + u_2 \ee_2) \cdot (v_1 \ee_1 + v_2 \ee_2) \\
          & = u_1 v_1 \ee_1 \cdot \ee_1 +  u_1 v_2 \ee_1 \cdot \ee_2 +
              u_2 v_1 \ee_2 \cdot \ee_1 + u_2 v_2  \ee_2 \cdot \ee_2 \\
          & = u_1 v_1  + u_2 v_2 \,.
\end{align*}
:::


The above proposition provides a natural way to define a scalar product in $\R^n$.


::: Definition
### Scalar product in $\R^n$

Let $\uu, \vv \in \R^n$ and denote their coordinates by
$$
\uu = (u_1, \ldots, u_n) \,, \quad
\vv = (v_1, \ldots, v_n) \,. 
$$
We define the scalar product between $\uu$ and $\vv$ by
$$
\uu \cdot \vv := \sum_{i=1}^n u_i v_i \,.
$$


:::

The scalar product in $\R^n$ is still bilinear and symmetric, as detailed in the following proposition:

::: Proposition
### Bilinearity and symmetry of scalar product in $\R^n$

Let $\uu, \vv, \ww \in \R^n$ and $\lambda \in \R$. Then
...
We can now define orthogonal vectors in $\R^n$.

::: Definition

Let $\uu, \vv \in \R^n$. We say that $\uu$ and $\vv$ are **orthogonal** if 
$$
\uu \cdot \vv = 0 \,.
$$
...
::: Proposition
### Differentiating the scalar product  {#proposition-diff-scalar}


Let $\g , \eeta \ \colon (a,b) \to \R^n$ be parametrized curves. The scalar map 
$$
...
Denote by 
$$
\g = (\gamma_1 ,\ldots , \gamma_n) \,, \quad \eeta = (\eta_1, \ldots , \eta_n)
$$
the coordinates of $\g$ and $\eeta$. Clearly the map
$$
t \mapsto \g \cdot \eeta = \sum_{i=1}^n \gamma_i \eta_i
$$
...
\begin{align*}
\frac{d}{dt} (\g \cdot \eeta) & = \frac{d}{dt} \left( \sum_{i=1}^n \gamma_i \eta_i \right) \\
& = \sum_{i=1}^n \frac{d}{dt} ( \gamma_i \eta_i ) \\
...
\dg (t) \,.
$$
The tangent vector measures the change of direction of a curve. Therefore the magnitude of $\dg$ can be interpreted as 
the rate of change, i.e. **speed**, of the curve.
...
The reason why we introduce unit-speed curves is because they make calculations easy. 
A crucial identity which allows to simplify calculations for unit-speed curves is given in
...
![If $\g$ is unit-speed then $\dg$ and $\ddg$ are orthogonal](/images/tangent_orthogonal.png){width=40%}
...
As we have observed in the Examples of @sec-introduction, there is in general no unique way to parametrize a curve. However we would like to understand when two parametrizations are related. In other words, we want to clarify the concept of **equivalence** of two parametrizations. First we need some notation:


::: Notation 

1. Let $X,Y,Z$ be sets and 
$$
f \colon X \to Y \,, \quad g \colon Y \to Z
$$
two maps. The **composition** of $f$ and $g$ is the map
$$
g \circ f \colon X \to Z  \,, \quad (g \circ f)(x) := g(f(x))
$$

2. The **identity map** on $X$ is denoted by 
$$
{\id}_{X} \colon X \to X \,, \quad  {\id}_{X} (x) := x \,, \quad \forall \, x \in X \,.
$$
The identity in $\R$ will just be denoted by $\id$.

3. The function $f \colon X \to Y$ is **invertible** if there exists
a function $g \colon Y \to X$ such that
$$
g \circ f = {\id}_{X} \,, \quad 
f \circ g = {\id}_{Y} \,,
$$
The map $g$, if it exists, is called the **inverse** of $f$ and is denoted by
$$
g:= f^{-1} \,.
$$

4. It is elementary to check that the inverse is unique if it exists.
...
1. $\f$ is invertible: There exists a map 
$$
\f^{-1} \ \colon (\tilde{a},\tilde{b}) \to (a,b)
$$ 
such that
$$
...
Since $\f$ is invertible with smooth inverse, $\tg$ is 
a reparametrization of $\g$
$$
\g(t) = \g( \f( \f^{-1}(t) ) ) = \tg ( \f^{-1}(t) ) \,, \quad 
\forall \, t \in (a,b) \,.
$$
...
1. Given a parametrized curve $\g$, this identifies a 1D shape $\Gamma \subset \R^n$ defined by
$$
\Gamma := \{ \g(t) \, \colon \, t \in (a,b) \} \,.
$$
$\Gamma$ is called the support of $\gamma$. 

2. A reparametrization $\tg$ is just an equivalent way to describe $\Gamma$. 

3. For $\g$ and $\tg$ to be reparametrizations of each other, there must exist a smooth rule $\f$ (the diffeomorphism) to switch from one to another, according to formula (@eq-reparametrization). This concept is sketched in @fig-reparametrization.

:::



![Sketch of 1D shape $\Gamma$ parametrized by $\g$ and $\tg$. We can switch parametrization by means of the diffeomorphism $\f$.](/images/reparametrization.png){#fig-reparametrization width=40%}
...
![Unit circle with usual parametrization $\g$, and with reversed orientation $\tg$](/images/circle_orientation.gif){width=40%}
...
![Unit circle with usual parametrization $\g$, and with reversed orientation $\tg$](/images/circle_orientation.png){width=40%}
...
![Lemniscate curve $\g$ and Lemniscate at double speed $\tg$](/images/lemniscate_animated.gif){width=40%}
...
![Lemniscate curve](/images/lemniscate_static.png){width=40%}
...
For a curve $\g$ we wish to find a reparametrization $\tg$ which is unit-speed:
$$
\norm{ \dtg } = 1 \,, \quad  \forall \, t \in (a,b) \,.
$$
We will see that this is possible if and only if the curve $\g$ is regular, in the sense of Definition \ref{definition-regular} below.




::: Definition
### Regular curve  {#definition-regular}

Let $\g \ \colon (a,b) \to \R^n$ be a parametrized curve. We say that:

1. $\g(t_0)$ is a **regular point** if 
$$
\dg(t_0) \neq 0 \,.
$$
2. A point $\g(t_0)$ is **singular** if it is not regular.
3. $\g$ is **regular** if every point of $\g$ is regular, that is,
$$
...
$$
\Gamma := \{ (x,y) \in \R^2 \, \colon \,  y=x^2 , \, -1 < x < 1\} \,.
$$
Both curves $\g, \eeta \ \colon (-1,1) \to \R^2$
$$
\g (t) = (t,t^2) \,, \quad 
\eeta(t) = (t^3, t^6) \,.
$$
are parametrizations of $\Gamma$. However $\eeta$ is not a reparametrization of $\g$.

> Indeed, suppose by contradiction there exist a diffeomorphism 
$$
\phi \colon (-1,1) \to (-1,1) 
$$
such that 
$$
\eeta (t) = \g (\phi(t)) \,, \quad \forall \, t \in (-1,1) \, .
$$
Substituting the definitions of $\g$ and $\eeta$ we obtain
$$
(t^3,t^6) = (\f(t), \f(t)^2) \,, \quad \forall \, t \in (-1,1)\,,
$$
which forces
$$
\f(t) = t^3 \,, \quad \forall \, t \in (-1,1)\,.
$$
Note that $f$ is invertible in $(-1,1)$ with inverse
$$
\f^{-1}(t) = \sqrt[3]{x} \,.
$$
However $\f^{-1}$ is not smooth at $t=0$, and therefore $\f$ is not a diffeomorphism. Alternatively we could have just noticed that 
$$
\dot \f (t) = 3t^2 \quad \implies \quad \dot\f(0) = 0 \,, 
$$
and therefore $\f$ cannot be a diffeomorphism due to Lemma \ref{lemma-diffeomorphism}.

To understand what is going on with the two parametrizations, let us look at the derivatives:
$$
\dg (t) = (1,2t) \,, \quad \dot{\eeta} (t)= (3t^2,6t^5) \,.
$$
We notice a difference:

- $\g$ is a regular parametrization, as the first component of $\dg$ is non-zero and so $\dg \neq 0$.
- $\eeta$ is regular if and only if $t \neq 0$.
...
![Left: Animation of $\g(t) = (t,t^2)$. Right: Animation of $\eeta(t) = (t^3,t^6)$.](/images/singular_curve_animated.gif){width=40%}
...
![Parabola $\Gamma$](/images/singular_curve.png){width=40%}
...
The previous example shows that, altough $\g$ and $\eeta$ describe the same parabola $\Gamma$, they are not reparametrizations of each other. We have seen that this is due to the fact that $\g$ is regular, while $\eeta$ is not. Indeed we can prove that regularity is invariant by reparametrization.
...
$$
As $\f$ is a diffeomorphism, by Lemma \ref{lemma-diffeomorphism} it holds
$$
\dot \f (t) \neq 0 \,, \quad \forall\, t \in (\tilde{a},\tilde{b})  \,,
$$
Therefore
$$
\dtg(t) \neq 0 \quad \iff \quad \dg (\f(t)) \neq 0 \,.
$$ {#eq-regularity-invariant-1}
Since $\g$ is regular we infer
$$
\dg (\f(t)) \neq 0 \,, \quad \forall\, t \in (\tilde{a},\tilde{b}) \,.
$$
From (@eq-regularity-invariant-1) we conclude that $\tg$ is regular.
:::
...
Let $\g \colon (a,b) \to \R^n$. A **unit-speed reparametrization** of $\g$ is a reparametrization $\tg \colon (\tilde{a},\tilde{b}) \to \R^n$ which is unit-speed, that is,
$$
...
s(t):=\int_{t_0}^t \norm{ \dg (\tau) } \, d\tau \,,
$$
for some arbitrary $t_0 \in (a,b)$ fixed. Notice that
$$
\dot{s}(t) = \norm{ \dg (t) } \,.
$$
Therefore
$$
\g \, \text{ regular}  \quad \iff \quad
\dot{s}(t) \neq 0 \,.
$$
In this case the arc-length $s$ is a diffeomorphism by the Inverse Function Theorem.
As it turns out, all the unit-speed reparametrizations of $\g$ are of the form
$$
\tg = \g \circ \psi  \,, \qquad \psi := \pm s^{-1} + c
$$
The above statements will be proved in Theorem \ref{theorem-regular-unit-speed} and Theorem \ref{theorem-arc-length-unit} below.
...
*Step 1. Direct implication.*  Assume $\g \ \colon (a,b) \to \R^n$ is regular, that is,
$$
\dg (t) \neq 0 \,, \quad \forall \, t \in (a,b)\,.
$$
...
$$
for some $\tilde{a}< \tilde{b}$. 
Define the reparametrization map $\f$ as
$$
\f := s^{-1}
$$
and the corresponding reparametrization $\tg$ of $\g$ as
$$
...
$$
where in the last equality we used (@eq-derivative-arc). Taking the absolute value of the above yields
$$
 \norm{ \dg(t) }   = \norm{\dtg(s(t))} \, \norm{ \dg(t) } \,.  
$${#eq-derivative-arc-1}
...
*Step 2. Reverse implication.*  Suppose there exists a unit-speed reparametrization of $\g$ denoted by 
$$
...
$$ {#eq-derivative-arc-3}
Hence none of terms on the LHS can be zero, meaning that
$$
\dg (\f(t))  \neq 0 \,, \quad \forall \, t \in (\tilde{a},\tilde{b}) \,.
$$
...
Let $\g \ \colon (a,b) \to \R^n$ be a regular curve. Let $\tg \ \colon (\tilde{a},\tilde{b}) \to \R^n$ be a reparametrization of $\g$, that is,
$$
...
::: Proof
*Step 1. First Point.* Assume $\tg$ is a unit-speed reparametrization of $\g$: such $\tg$ exists by Theorem \ref{theorem-regular-unit-speed}, since $\g$ is assumed to be regular. This means there exists a reparametrization map $\f$ such that
$$
...
$$
\dot s(t) = |\dot \f(t) |  \quad  \forall \, t \in (a,b)
$${#eq-proposition-unit-speed-2bis}
As $\f$ is a diffeomorphism, by Lemma \ref{lemma-diffeomorphism} we have
$$
\dot \f(t) \neq 0  \quad  \forall \, t \in (a,b) \,.
$$
By continuity of $\dot \f$ and the Mean Value Theorem we conclude that
either
$$
\dot \f(t) > 0  \quad  \forall \, t \in (a,b) \,.
$$
or 
$$
\dot \f(t) < 0  \quad  \forall \, t \in (a,b) \,.
$$
Therefore (@eq-proposition-unit-speed-2bis) reads either
$$
\dot s(t) = \dot \f(t)   \quad  \forall \, t \in (a,b)
$$
or
...
*Step 2. Second Point.*  Let $\tg$ be reparametrization of $\g$
$$
\g(t) = \tg (\f(t)) \,, \quad \forall \, t \in (a,b) 
$${#eq-proposition-unit-speed-2tris}
with 
$$
\f := \pm s + c
$$
for some $c \in \R$. Differentiating (@eq-proposition-unit-speed-2tris) we get
\begin{align*}
\dg (t) & = \dtg( \f(t) ) \dot\f(t) \\
        & = \pm \dtg( \f(t) ) \dot s(t) \\
        & =  \pm \dtg( \f(t) ) \norm{ \dg(t) }
\end{align*}
where in the last equality we used the Fundamental Theorem of Calculus and
definition of $s$. Taking the absolute values
$$
\norm{\dg(t)} = \norm{ \dg(\f(t)) } \norm{ \dg(t)} \,.
$$
Since $\g$ is regular we have $\norm{\dg(t)} \neq 0$. Hence we can divide by
$\norm{\dg(t)}$ and obtain that
$$
\norm{ \dg(\f(t)) } = 1 \,, \quad \forall \, t \in (a,b) \,.
$$
As $\f$ is invertible, the above is equivalent to
$$
 \norm{ \dtg( t )}  = 1 \, \quad \forall \, t \in (\tilde{a},\tilde{b})  \, ,
$$
...
The above notation might seem confusing, but it actually makes
a lot of sense in the long run, as calculations get heavier.
Accordingly we have the following notation for 
the arc-length reparametrization.
...
::: Example 
### Arc-length reparametrization of Circle {#example-circle-arc}
...
\dg(t) = (1,2t,3t^2) \neq \zero \,,
$$
meaning that $\g$ is regular. In particular
$$
...
$${#eq-twisted-arc}
Since $\g$ is regular, by Theorem \ref{theorem-arc-length-unit} 
we know that $\g$ admits the unit-speed reparametrization
$$
\tg :=  \g \circ s^{-1}
$$
with $s^{-1}$ the inverse of the arc-length function. 
It can be shown that the integral 
at (@eq-twisted-arc) cannot be written in terms of elementary functions. 
Therefore there are not explicit formulas for $s$ and $s^{-1}$.
As a consequence the unit-speed parametrization $\tg$ cannot be computed explicitly
 in this case.
...
1. Curves which are infinite, or **unbounded**. This is for example the parabola
$$
\g(t) := (t,t^2) \,, \quad \forall \, t \in \R \,,
$$

2. Curves which are finite and have end-points, such as the semi-circle
$$
\g(t) := (\cos(t),\sin(t)) \,, \quad \forall \, t \in [0,\pi] \,,
$$

3. Curves which form **loops**, such as the circle
$$
...
1. $\g$ is unbounded: If define $\tg$ as the restriction of $\g$ to the time interval $[1,\infty)$, then $\tg$ is unbounded. A point which starts at $\g (1) = (0,0)$ goes towards infinity. 

2. $\g$ contains a loop: If we define $\tg$ as the restriction of $\g$ to the time interval $[-1,1]$, then $\tg$ is a closed loop starting at $\g (-1) = (0,0)$ and returnning at $\g (1) = (0,0)$. 
...
![Animated plot of curve $\g(t) = (t^2-1, t^3-1)$ for $t \in [-2,2]$](/images/loop_animated.gif){width=40%}
...
![Plot of curve $\g(t) = (t^2-1, t^3-1)$ for $t \in [-2,2]$](/images/loop_static.png){width=40%}
...
Let $\g \ \colon \R \to \R^n$ be a parametrized curve. We say that $\g$ is **closed** if they hold

1. $\g$ is not constant,
2. $\g$ is T-periodic for some $T \neq 0$.
...
1. If $\g$ is $T$-periodic, then a point moving around $\g$ returns to its starting point after time $T$.
     ...
     >This is exactly the definition of $T$-periodicity: let $p = \g(a)$ be the point in question, then
     $$
     \g(a + T) = \g(a) = p 
     $$ 
...
T:=b-a \,.
$$
Then $\g$ can be extended to a smooth $T$-periodic curve $\tg \ \colon \R \to \R^n$ defined by
$$
...
$$
see figure below. In this way $\tg$ is $T$-periodic and smooth. 

    > Note that 
    assumption (@eq-smooth-extension) must hold for all $k \in \N$ for the 
    extension $\tg$ to be smooth. As a counterexample consider 
    $f(x):=x^2$ for $x \in [-1,1]$. As seen by plotting $f$, it is clear that $f$ cannot be extended to a smooth periodic function. And indeed in this case (@eq-smooth-extension) is violated, because
    $$
    f(-1) = f(1) = 1 \,, \quad f'(-1) = -2 \neq 2 = f'(1)
    $$
    showing that the periodic extension is continuous but not differentiable.
...
6. If $\g$ is $T$-periodic then $\g$ is $(kT)$-periodic, for all $k \in \Z$.
...
![The points $t \in \R$ and $\tilde{t} \in [a,b]$ from Point 3 in Remark \ref{remark-closed}. In this skecth $t = \tilde{t} + 3 T$, with $T = b-a$.](/images/periodic_extension.png){width=40%}
...
P = \min S  \,.
$$
Since $P = \inf S$, the above is equivalent to showing that 
...
Since $P = \inf S$, there exists an infimizing sequence $\{T_n\}_{n \in \N} \subset S$ such that
$$
T_n \to P \,.
$$
...
*Proof of Point 1.* As $T_n \in S$, by definition $\g$ is $T_n$-periodic. Then
$$
\g(t) = \g (t + T_n)  \,, \quad \forall \, t \in \R \,,  \,\, n \in \N \,.
$$
...
*Proof of Point 2.* We have shown that $\g$ is $P$-periodic. Therefore
$$
P \in S \quad \iff \quad P > 0 \,.
$$
...
![For each $t \in \R$ there exist unique $k_n \in \Z$ and $\tilde{t}_n \in [0,T_n]$ such that $t=\tilde{t} + k_n T_n$. In this skecth $k_n =3$.](/images/archimedean.png){width=40%}
...
3. Consider again the curve from Example \ref{example-curve-loop} 
$$
...
![Limaçon curve](/images/limacon.png){width=40%}