...
**Question.** Compute the matrix of the Weingarten map of the following surface chart
$$
\sss (u, v) = \left(u-v, u+v, u^{2}+v^{2}\right) \,.
$$
...
\right)  \,.
$$
Thus, $\mathcal{F}_1^{-1}$ is symmetric. Since 
$$
\mathscr{F}_2 =
\left( 
\begin{array}{cc}
...
The matrix version of Theorem \ref{theorem-weingarten-diagonal-bis} is given in the following Corollary.


::: {.Corollary #corollary-weingarten-diagonal-bis}

Let $\SSS$ be orientable, $\sss$ is a regular chart at $\pp$. Let $\WW$ the matrix of the Weingarten map $\WW_{\pp,\SSS}$
with respect to the basis $\{\sss_u,\sss_v\}$ of $T_{\pp}\SSS$. Let $\kappa_1,\kappa_2,\mathbf{t}_1,\mathbf{t}_2$ be as in Theorem \ref{theorem-weingarten-diagonal-bis}.
...
By Theorem \ref{theorem-weingarten-diagonal-bis} we have
$$
\WW_{\pp,\SSS} (\mathbf{t}_1) = \kappa_1 \mathbf{t}_1 \,, 
\quad \WW_{\pp,\SSS} (\mathbf{t}_2) = \kappa_2 \mathbf{t}_2 \,.
$$
...
Corollary \ref{corollary-weingarten-diagonal-bis} gives an explicit way
...
1. Compute the eigenvalues of $\WW$, by solving for $\lambda$ the equation
$$
\det(\WW - \lambda I) = 0 \,.
$$
...
2. Find scalars $\lambda$, $\mu$ which solve the linear system
$$
(\WW - \kappa_i I) 
\left( 
\begin{array}{c}
...
3. The principal vector(s) associated to $\kappa_i$ is  
$$
...
By Corollary \ref{corollary-weingarten-diagonal-bis} we have
$$
\WW = P^{-1} D P \,, \quad 
D = 
\left( 
...
::: Example
### Curvatures of Sphere  {#example-sphere-principal}
