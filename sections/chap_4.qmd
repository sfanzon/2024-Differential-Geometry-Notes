...
::: Warning
 ...
 Changing parametrization can change the local form of $\FF_1$, as seen in the next two examples. This is not surprising, due to Proposition \ref{proposition-fff-reparametrization}.
...
where $\widetilde{\mathscr{F}}_1$ is the first fundamental form of $\widetilde{\sss} = f \circ \sss$. Assume that $\hat{\sss} \colon \hat{U} \to \SSS$ is another chart of $\SSS$. By Remark \ref{proposition-fff-reparametrization}, the fundamental forms of $\hat{\sss}$ and $\sss$ are related by
$$
...
$$
showing that $\sss^{\dagger}$ is a reparametrization of $\widetilde{\sss}$ with transition map $\Phi$. Therefore, by Proposition \ref{proposition-fff-reparametrization}, the fundamental forms of $\sss^{\dagger}$ and $\widetilde{\sss}$ are related by
$$
...
\tg(t) \cdot \dg(t) = 0 \,,
$$
since $\NN$ is normal to $\SSS$ at $\pp = \g(t)$, and $\dg(t) \in T_{\pp}(\SSS)$.
...
**Solution.** From Example \ref{example-sphere-principal-bis}, the second fundamental form of $\sss$ is
$$
{\FF}_2 = \cos^{2}(v) du^2 + dv^2 \,.
$$
...
\begin{align*}
\dg(t) & = \frac{d}{dt} ( \cos(u(t)) \cos(v(t)), \sin(u(t)) \cos(v(t)), \sin(v(t)) ) \\
...
$$
where in the last equality we used that $\g$ is unit speed. By Theorem \ref{theorem-normal-curvature}, the normal
curvature of $\g$ is
$$
\kappa_{n}=\cos^{2}(v) \dot{u}^{2}+\dot{v}^{2} = 1 \,.
$$
...
Let us consider again the chart for the unit sphere
$$
\sss(u, v)=(\cos (u) \cos (v), \sin (u) \cos (v), \sin (v)) \,,
$$
as seen in Example \ref{example-curves-on-sphere}. By Example \ref{example-sphere-principal-bis}, the principal curvatures of the unit sphere are
$$
\kappa_1 = \kappa_2 =  1 \,.
$$
...
We conclude with stating two important and powerful Theorems:
...
The question is whether there are relations between the first and second fundamental forms. As it turns out, all the relations are encoded in two sets of equations:
...
**Question.** Consider the two surfaces $\SSS$ and $\wSSS$ charted by, respectively
\begin{align*}
\sss(u,v) & = (\cos(u)v, \sin(u)v, \log (v)), \,\, v > 0\,,  \\
...
$$
In particular, the first fundamental form of $f \circ \sss$ is the same as the one of $\Phi \circ \wsss$. But the first fundamental form of $\Phi \circ \wsss$ is
$$
(J\Phi)^T \wFF_1 J\Phi = \wFF_1 \,,
$$
since $\wFF_1$ is diagonal, and $(J\Phi)^T J\Phi = I$.
We conclude that 
$$
\FF_1 = \wFF_1 \,,
$$
a clear contradiction. Therefore, $\SSS$ and $\wSSS$ are not locally isometric.
