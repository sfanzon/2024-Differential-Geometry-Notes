...
![Sketch of a curve $\g$.](/images/curves_example.png){width=40%}


Surfaces are 2D objects in $\R^3$. There are two directions in which one can move on a surface. 


![Sketch of a surfaces: Sphere, Torus, Möbius band.](/images/surfaces_example.png){width=40%}
...
![A curve $\Gamma$ can be described by a function $\g \colon (a,b) \to \Gamma$.](/images/curves_example_2.png){width=40%}
...
![A surface $\SSS$ can be described by a family of charts $\sss_i \colon U_i \to \SSS$ with $U_i \subseteq \R^2$ open set.](/images/surfaces_example_2.png){width=40%}
...
![Sketch of the surface $\SSS$ and chart $\sss \colon U \to \sss(U) \subseteq \SSS$. The set $U \subseteq \R^2$ is open in $\R^2$ and $\sss(U)$ is open in $\SSS$. This means there exists $W$ open in $\R^3$ such that $\sss(U) = \SSS \cap W$.](/images/surfaces_example_3.png){#fig-def-surface width=40%}
...
![A plane $\p$ is a surface with atlas containing a single chart $\sss \colon \R^2 \to \p$.](/images/surface_plane.png){#fig-plane-surface width=40%}
...
![Unit cylinder $\SSS$ is a surface with atlas $\mathcal{A} = \{\sss_1,\sss_2\}$. Depicted are the images $\sss_1(U_1)$ and $\sss_2(U_2)$. ](/images/surface_cylinder.png){#fig-cylinder-surface width=40%}
...
![The circular cone is not a surface. This is because $\SSS \smallsetminus \{\zero\}$ is disconnected.](/images/surface_cone.png){#fig-surface-cone width=40%}
...
![Spherical coordinates in $\R^3$.](/images/spherical_coordinates.png){#fig-spherical-coordinates width=40%}
...
![Equator Line, Greenwich Meridian, Date Line, North and South Poles on the sphere.](/images/earth.png){#fig-earth width=40%}
...
**Question.**  Define the following collection of charts on the sphere $\sphere^2$
$$
...
\end{align*}
Prove that $\mathcal{A}$ is a regular atlas.

**Solution.** Let us check that $\sphere^2$ is a surface:
...
![Schematic illustration of surface chart $\sss$ and reparametrization $\widetilde{\sss}$.](/images/surface_reparametrization.png){width=40%}
...
::: Theorem
### Reparametrizations of regular charts are regular {#theorem-reparametrization}
...
$$
for some $i \neq j$.
It is natural to ask whether the charts $\sss_i$ and $\sss_j$ are reparametrizations of each other on the overlapping region $I$, see @fig-transition-maps. This is indeed the case, see Theorem \ref{theorem-transition-reparam} below. Such reparametrization is called a *transition map*.


![If the two regular charts $\sss$ and $\widetilde{\sss}$ have overlapping image, then they are reparametrization of each other, through a transition map $\Phi =  {\sss}^{-1} \circ \wsss$.](/images/surfaces_transition_map.png){#fig-transition-maps width=40%}
...
The immediate consequence of Theorem \ref{theorem-transition-diffeo} is that transition maps are reparametrizations. 
...
Theorem \ref{theorem-transition-reparam} guarantees that $\Phi$ is a reparametrization map. Therefore $\sss$ and $\widetilde{\sss}$ are reparametrization of each other, with 
$$
\widetilde{\sss} = \sss \circ \Phi \,.
$$
...
Theorem \ref{theorem-transition-reparam} demonstrates that there is no single preferred way to parametrize a surface: When two regular charts overlap, they are reparametrizations of each other in the overlapping region.
...
![The function $f \colon \SSS_1 \to \SSS_2$ is smooth at $\pp$, if the vector valued function $\sss_2^{-1} \circ f \circ \sss_1$ is smooth.](/images/surface_smooth_map.png){#fig-smooth-functions width=40%}
...
4. Definition \ref{definition-smooth-map-surface} is well-posed: Smoothness of $f$ does not depend on the specific choice of charts $\sss_1$ and $\sss_2$.  
    ...
    $$
    As $\SSS_1$ and $\SSS_2$ are regular surfaces, by Theorem \ref{theorem-transition-diffeo} there exist open sets
    $$
...
$$
By Theorem \ref{theorem-transition-diffeo}, the map $\Phi$ is differentiable. As $\Phi = \sss^{-1} \circ \wsss$, the proof is concluded.
...
$\sss_2$. Since $\sss_2$ is regular, by Theorem \ref{theorem-reparametrization} we deduce
...
## Tangent plane {#sec-tangent-space}
...
![Tangent plane $T_{\pp} \SSS$ of surface $\SSS$ at the point $\pp$. A tangent vector $\vv$ has to satisfy $\vv = \dg(0)$, for some smooth curve $\g \colon (-\e,\e) \to \SSS$ such that $\g(0)= \pp$.](/images/tangent_space.png){#fig-tangent-plane width=40%}



Let us start with the most basic example: We want to compute the tangent plane to an open set in $\R^2$. 
...
$$
for a pair of smooth functions $u,v \colon (-\e,\e) \to \R$.
...
To visualize the geometric ideas of this part of the proof, see @fig-curves-surface. Let $\pp$ in $\SSS=\sss(U)$, and assume given a smooth curve $\g \colon (-\e,\e) \to \R^3$ such that $\g(0) = \pp$ and
$$
\g(t) \in \SSS \,, \quad \forall \, t \in (-\e,\e) \,.
$$
Denote the coordinates of the chart $\sss \colon U \to \R^3$ by
$$
...
![Image associated with the Proof for Lemma \ref{lemma-curve-S}. The smooth coordinates $\eeta(t) = (u(t),v(t))$ are constructed by setting $\eeta := F^{-1} \circ \pi \circ \g$.](/images/curves_surface.png){#fig-curves-surface width=40%}
...
Let $\sss \colon U \to \SSS$ be a regular chart.

- First, suppose $\vv \in T_{\pp} \SSS$. By definition of tangent plane, there exists 
...
The tangent plane is a vector space isomorphic to $\R^2$, that is,
$$
T_{\pp} \SSS \simeq \R^2 \,,
$$
...
By Theorem \ref{theorem-tangent-plane} we have that
$$
...
$\sss_u$ and $\sss_v$. Therefore it is orthogonal to
...
::: Remark 
### Equation of affine tangent plane
...
The vector $\nn$ is orthogonalto $T_{\pp}\SSS$. In particular, the equation for the affine tangent plane $\pp + T_{\pp} \SSS$ is
$$
...
```{python}
#| echo: false
#| fig-cap: "The Paraboloid in the Example."
...
\widetilde{\sss} = \sss \circ \Phi \,, 
$$
with $\Phi \colon \widetilde{U} \to U$ diffeomorphism. As stated in Thorem \ref{theorem-reparametrization}, we have
$$
\widetilde{\sss}_{\tilde{u}} \times \widetilde{\sss}_{\tilde{v}}
...
d_{\pp} f (\vv) = (f \circ \g)'(0)\,.
$$
Method to construct $\g$: Following the proof of Theorem \ref{theorem-differential-smooth}, one can proceed as follows: 
...
2. **By using the matrix representation:** Let $\vv \in T_{\pp} \SSS$
    ...
    - Compute 
    $$
    d_{\pp} f (\vv) = \tilde{\lambda} \wsssu + \tilde{\mu} \wsssv
    $$
    where the coefficients $\tilde{\lambda}$ and $\tilde{\mu}$ are
    $$
    \left(
    \begin{array}{c}
    \tilde{\lambda} \\
...
$\nabla f(\pp) \perp T_{\pp} \SSS_f$, and
...
![Classification of quadrics: Ellipsoid and Hyperboloid of 1 sheet.](/images/quadrics_1.png){#fig-quadrics-1 width=40%}

![Classification of quadrics: Hyperboloid of 2 sheets and Elliptic Paraboiloid.](/images/quadrics_2.png){#fig-quadrics-2 width=40%}

![Classification of quadrics: Hyperbolic Paraboloid and Quadric Cone.](/images/quadrics_3.png){#fig-quadrics-3 width=40%}

![Classification of quadrics: Elliptic Cylinder and Hyperbolic Cylinder.](/images/quadrics_4.png){#fig-quadrics-4 width=40%}

![Classification of quadrics: Parabolic Cylinder and Plane.](/images/quadrics_5.png){#fig-quadrics-5 width=40%}

![Classification of quadrics: 2 parallel planes and 2 intersecting planes.](/images/quadrics_6.png){#fig-quadrics-6 width=40%}
...
![The unit cylinder is a ruled surface with base curve the unit circle, and rulings given by vertical lines.](/images/surfaces_ruled.png){#fig-cylinder-ruled width=40%}
...
![The Möbius band is a ruled surface with base curve $\g$ and rulings given by rotating vertical lines.](/images/surfaces_ruled_2.png){#fig-mobius-ruled width=40%}
...
![The Catenoid is the surface of revolution obtained by rotating the catenary about the $z$-axis.](/images/surface_catenoid.png){#fig-catenoid width=40%}
...
![Sketch of a local isometry $f$ between $\SSS$ and $\widetilde{\SSS}$. The scalar product between tangent vectors $\vv$ and $\ww$ is preserved by $d_{\pp}f$.](/images/fff_local_isometry.png){width=40%}
...
$$
Since $f$ is a local isometry, then it is also a local diffeomorhims by Proposition \ref{prop-isometry-diffeo}. In particular, Proposition \ref{proposition-f-chart} ensures that $\widetilde{\sss}$ is a regualar chart of $\widetilde{\SSS}$ at $f(\pp)$. Now, recall the statement of Theorem \ref{theorem-differential-smooth}: if
$$
\widetilde{\sss} ( \alpha(u,v), \beta(u,v) ) = f ( \sss (u,v) ) \,,
$$
...
\end{align*}
As discussed above, since $\widetilde{\sss} = f \circ \sss$, by Theorem \ref{theorem-differential-smooth} we get
$$
...
![A simple closed curve $\g$ in the plane $\SSS$ can be shrunk continuously into a point without leaving $\SSS$. A parallel $\g$ of the Cylinder $\wSSS$ cannot be shrunk continuously into a point without leaving $\wSSS$.](/images/simply_connected.png){#fig-simply-connected width=40%}
...
![Sketch of angle $\theta$ between two vectors $\vv,\ww$ in $T_{\pp} \SSS$.](/images/fff_angle_vectors.png){width=40%}
...
![Sketch of angle $\theta$ between two curves $\g$ and $\tg$ on $\SSS$.](/images/fff_angle_curves.png){width=40%}
...
![Sketch of conformal map $f$ between $\SSS$ and $\widetilde{\SSS}$. The angles between tangent vectors are preserved by $d_{\pp} f$.](/images/fff_conformal_map.png){width=40%}
...
![Stereographic projection map from the North pole $N = (0,0,1)$. Let $P \in \sphere^2$. The line through $N$ and $P$ intersects the plane $\{z=0\}$ at the point $P'$.](/images/stereographic_4.png){#fig-stereographic-4 width=40%}
...
![Stereographic projection of the Earth. Image from [Wikipedia](https://en.wikipedia.org/wiki/Stereographic_projection)](/images/stereographic_2.png){#fig-stereographic-1 width=40%}

![The Stereographic projection is a conformal map: Angles and shapes are preserved. However areas are magnified away from the North pole.](/images/stereographic_3.png){#fig-stereographic-2 width=40%}
...
![Mercator projection of the Earth. This is the most frequently used map of the Earth. Image from [Wikipedia](https://en.wikipedia.org/wiki/Mercator_projection).](/images/mercator_2.png){#fig-mercator width=40%}

![The Mercator Projection is a conformal map: As such it preseves angles and shapes. However areas are distorted away from the Equator. Image from [Wikipedia](https://en.wikipedia.org/wiki/Mercator_projection).](/images/mercator_3.png){#fig-mercator-2 width=40%}
...
![The area $A_{\sss}$ of a small region $\sss(R)$ on a surface $\SSS$ can be approximated by the area of the parallelogram of sides $\sss_u$ and $\sss_v$. Such area is $\| \sss_u \times \sss_v \|$.](/images/fff_area.png){#fig-area width=40%}
...
![Sketch of equiareal map $f$ between the surfaces $\SSS$ and $\widetilde{\SSS}$. The area of the region $R$ is the same as the area of $f(R)$.](/images/fff_equiareal.png){#fig-equiareal-map width=40%}
...
![Construction of the Archimedes Map between the Sphere and the Cylinder.](/images/archimedes.png){#fig-archimedes width=40%}
...
![Lambert cylindrical projection of the Earth. Images from [Wikipedia](https://en.wikipedia.org/wiki/Lambert_cylindrical_equal-area_projection).](/images/lambert_2.png){#fig-lambert width=40%}


![The Lambert cylindrical projection is an equiareal map. As such it preseves areas. However angles and shapes are distorted. Image from [Wikipedia](https://en.wikipedia.org/wiki/Lambert_cylindrical_equal-area_projection).](/images/lambert_3.png){#fig-lambert-2 width=40%}
...
![The point $\sss(u_0 + \Delta u, v_0 + \Delta v)$ on $\SSS$ deviates from $T_{\pp}\SSS$ by a quantity $\delta$.](/images/sff_sketch.png){#fig-sff-sketch width=40%}
...
![The Gauss map ${\mathcal{G}}_{\SSS}$ of $\SSS$ is defined as ${\mathcal{G}}_{\SSS}(\pp):= \NN(\pp)$. Note that ${\mathcal{G}}_{\SSS}(\pp) \in \sphere^2$.](/images/sff_gauss_map.png){width=40%}
...
![The Gauss map ${\mathcal{G}}_{\SSS}$ of a sphere is the identity.](/images/sff_gauss_sphere.png){#fig-gauss-sphere width=40%}


![The Gauss map ${\mathcal{G}}_{\SSS}$ of a plane is constant.](/images/sff_gauss_plane.png){#fig-gauss-plane width=40%}


![If $\SSS$ is the unit cylinder, the Gauss map ${\mathcal{G}}_{\SSS}$ maps $\SSS$ into the equator of $\sphere^2$.](/images/sff_gauss_cylinder.png){#fig-gauss-cylinder width=40%}
...
![We ca identify $T_{{\mathcal{G}}_{\SSS}(\pp)} \sphere^2$ with $T_{\pp} \SSS$. This is because $\mathcal{G}(\pp) \perp T_{{\mathcal{G}}_{\SSS}(\pp)} \sphere^2$ and $\mathcal{G}(\pp) = \NN(\pp)$.](/images/sff_weingarten.png){#fig-weingarten width=40%}
...
![Principal vectors of the unit cylinder.](/images/sff_principal_cylinder.png){#fig-cylinder-principal width=40%}
...
![](/images/surfaces_local_shape_1.png){width=40%}

![](/images/surfaces_local_shape_2.png){width=40%}
